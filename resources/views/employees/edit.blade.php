        ...
        <form method="POST" action="{{ route('employees.update', $employee) }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="text" name="department_name" value="{{ old('department_name', $employee->department_name) }}" required
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:border-blue-500">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <div>
                    ...
                    <input type="number" step="0.01" name="annual_salary" value="{{ old('annual_salary', $employee->annual_salary) }}"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:border-blue-500">
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="date" name="start_date" value="{{ old('start_date', $employee->start_date?->format('Y-m-d')) }}"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:border-blue-500">
                </div>
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="office_location" value="{{ old('office_location', $employee->office_location) }}"
                       class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:border-blue-500">
            </div>

            <div class="border-t border-gray-200 pt-6 mt-6">
                <h3 class="text-base font-semibold text-gray-900 mb-4 flex items-center gap-2">
                    <i class="fa-solid fa-user-lock text-blue-600"></i> Employee Login Account
                </h3>
                ...
                <div class="mb-4">
                    <label class="flex items-center gap-3 cursor-pointer">
                        <input type="checkbox" name="login_enabled" value="1" id="login-enabled-checkbox"
                               ...
                               {{ old('login_enabled', $employee->login_enabled) ? 'checked' : '' }}>
                        <span class="text-sm font-medium text-gray-800">Login Access Enabled</span>
                    </label>
                    <p class="text-xs text-gray-500 mt-1 ml-7">Disabling access removes the employee's login account.</p>
                </div>

                <div id="account-section" class="bg-gray-50 border border-gray-200 rounded-lg p-4 {{ old('login_enabled', $employee->login_enabled) ? '' : 'hidden' }}">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Email</label>
                            <input type="email" name="email" value="{{ old('email', $employee->email) }}" id="account-email"
                                   class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:border-blue-500">
                        ...
                        <div>
                            ...
                            <select name="role" id="account-role" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:border-blue-500">
                                ...
                                @foreach($roles as $role)
                                    <option value="{{ $role->code }}" {{ old('role', $employee->user?->roles->first()->code ?? '') == $role->code ? 'selected' : '' }}>{{ $role->name }}</option>
                                @endforeach
                            </select>
                    ...
                    <div id="account-company-field" class="mb-4 hidden">
                        ...
                        <select name="account_company_id" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:border-blue-500">
                            <option value="">Select company</option>
                            @foreach($companies as $company)
                                <option value="{{ $company->id }}" {{ old('account_company_id', $employee->user?->companies->first()->id ?? '') == $company->id ? 'selected' : '' }}>{{ $company->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div id="account-department-field" class="mb-4 hidden">
                        ...
                        <select name="account_department" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:border-blue-500">
                            ...
                            @foreach($departments as $department)
                                <option value="{{ $department->department_name }}" {{ old('account_department', $employee->user?->department_name ?? '') == $department->department_name ? 'selected' : '' }}>{{ $department->department_name }}</option>
                            @endforeach
                        </select>
