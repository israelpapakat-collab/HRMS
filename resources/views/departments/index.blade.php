<x-app-layout>
    ...
    <div class="bg-white rounded-lg shadow">
        <div class="p-4 border-b border-gray-200 flex items-center justify-between">
            <p class="text-gray-900">{{ $departments->total() }} departments</p>
            ...
            <table class="w-full">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-900 uppercase">Department Name</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse($departments as $dept)
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 text-sm text-gray-900">{{ $departments->firstItem() + $loop->index }}</td>
                            <td class="px-6 py-4 text-sm font-medium text-gray-900">{{ $dept->department_name }}</td>
                            <td class="px-6 py-4 text-sm text-gray-900">{{ $dept->employees_count }}</td>
                            <td class="px-6 py-4 text-sm">
                                <a href="{{ route('departments.edit', $dept) }}" class="text-blue-600 hover:text-blue-800 mr-3">Edit</a>
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-6 py-8 text-center text-gray-900">No departments found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
